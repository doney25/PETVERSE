...
const CheckoutPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const orderResponse = await axios.post(`${API_BASE_URL}/api/orders/placeOrder`, newOrder);
        
        if (orderResponse.status !== 201) throw new Error("Failed to create order");
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto py-8">
          ...
          <RadioGroup onValueChange={setPaymentMethod} className="space-y-4">
              <div className="grid grid-cols-1 gap-4">
                {[
                  ...
                  <Label key={option.value} className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-100">
        ...
        <Button type="submit" className="mt-1 min-w-full" onClick={handleSubmit}>
              Place Order
            </Button>
      </div>
    </>
